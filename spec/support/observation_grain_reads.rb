...
# aggregate) — and UP TO FOUR MORE for the cross-run flakiness block, which is the only grain here
...
module ObservationGrainReads
  ...
  # `[area, file, example, flakiness]` — the four grains, each an array of the statements matched.
  ...
  def flakiness_grain_reads(&) = observation_reads_by_grain(&)[3]
end

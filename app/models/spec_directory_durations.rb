...
class SpecDirectoryDurations
  ...
  Row = Struct.new(:path, :total_seconds, :recorded_count, :timed_count, :distinct_name_count,
    ...
    # How much of the area this total covers — through the same seam the other single-sided coverage
    # fractions are spelled through, so a complete area is visibly complete rather than merely
    # unannotated, and the rows on this panel cannot disagree with the files drilled out of them
    # about how the fraction is worded.

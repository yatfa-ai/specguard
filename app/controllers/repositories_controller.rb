...
class RepositoriesController < ApplicationController
  ...
  def show
    ...
    # discard it, on the very page whose stated rule — the API keys panel comment in
    # `show.html.erb`: *"controls and credential metadata are gated, the endpoint documentation and
    # connection-health stat above are not"* — is that credential metadata is gated. Inside the gate
    # the preload is still required: without it, listing keys is one user query per key. Asking
    # costs nothing — `repository_policy` is memoized and already
